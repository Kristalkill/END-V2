    ...
    async reaction_on_message(message: Message): Promise<void | Message | MessageReaction> {
        ...
        if (data && res) {
            ...
            if (command && prefix) {
                ...
                if (cool_down) return await this.client.embed.error(language.events.message.slow_mode.translate({
                ...
                if (this.client.owners.includes(message.author.id) === false) {
                    ...
                    if (user_permission) return await this.client.embed.error(language.events.message.no_enough_laws.translate({
                        need: user_permission,
                        user: `${message.member} you`
                    }), message);
                }
                if (bot_permission) return await this.client.embed.error(language.events.message.no_enough_laws.translate({
                    need: bot_permission,
                    user: `i`
                }), message)
            ...
            } else if (message.mentions.users.has(message.guild.me.id) === true) return await message.channel.send(
